...
class CustomDataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        return np.array(batch_images), to_categorical(self.label_encoder.transform(batch_labels), num_classes=len(self.classes))
...
def dataset_split(dataset_path: str, test_split: float = 0.2, val_split: float = 0.2):
    ...
    for item in os.listdir(dataset_path):
        ...
        if os.path.isdir(item_path):
            class_label = item  # Class label is the directory name
